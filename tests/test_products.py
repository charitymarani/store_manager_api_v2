...
class TestProducts(Testbase):
    ...
    def test_edit_product(self):
        with self.client:
            ...
            response = self.client.put(
                self.producturl+'/200',data=json.dumps(self.edit_data),
            ...
            print(response_data)
            self.assertEqual(response.status_code, 200)
            self.assertEqual("Product updated successfully!",response_data["response"]["message"])
            #Test empty data
            response1 = self.client.put(
                self.producturl+'/504',data=json.dumps({}),
                headers=dict(Authorization="Bearer " + token),
                content_type='application/json'
                )
            response_data1=json.loads(response1.data)
            ...
            response2 = self.client.put(
                self.producturl+'/200',data=json.dumps(self.empty_data_fields),
                headers=dict(Authorization="Bearer " + token),
                content_type='application/json'
                )
            response_data2=json.loads(response2.data)
            self.assertEqual("All fields cannot be empty enter data to edit",response_data2["message"])
            ...
            self.assertEqual("Product does not exist",response_data3["message"])
            # Register attendant
            self.client.post(
                self.signupurl, headers=dict(Authorization="Bearer " + token),
            ...
            response4 = self.client.put(
                    self.producturl+'/504',data=json.dumps(self.edit_data),
                        headers=dict(Authorization="Bearer " + tokenatt),
                        content_type='application/json'
                        )
            response_data4=json.loads(response4.data)
            self.assertEqual("Only admin can edit a product",response_data4["message"])
    ...
    def test_delete_product(self):
        with self.client:
            ...
            print(result)
            self.assertEqual("product has been deleted!",result["response"]["message"])
            self.assertEqual(response.status_code,200)
            #Test nonexistent product
            response2=self.client.delete(self.producturl+'/700',headers=dict(Authorization="Bearer " + token))
            result2=json.loads(response2.data)
            self.assertEqual("product_id does not exist in our records",result2["message"])
            ...
            response3=self.client.delete(self.producturl+'/504',headers=dict(Authorization="Bearer " + tokenatt))
            result3=json.loads(response3.data)
            self.assertEqual("Only admin can delete a product",result3["message"])
